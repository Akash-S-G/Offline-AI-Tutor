...
async def voice_websocket_proxy(websocket: WebSocket):
    ...
    logger.info("[VOICE_PROXY] Connection opened")
    
    target_url = settings.voice_service_url.replace("http://", "ws://").replace("https://", "wss://")
    target_url += "/voice/stream"
    logger.info(f"[VOICE_PROXY] Proxying to {target_url}")
    ...
    try:
        async with websockets.connect(target_url) as backend_ws:
            ...
            async def client_to_backend():
                ...
                try:
                    while True:
                        ...
                        try:
                            ...
                            if "session_id" in event:
                                ...
                                logger.info(f"[VOICE_PROXY] Session={session_id}")
                            elif "payload" in event and isinstance(event["payload"], dict):
                                s_id = event["payload"].get("session_id")
                                if s_id:
                                    session_id = s_id
                                    logger.info(f"[VOICE_PROXY] Session={session_id}")
                            
                            if event_type:
                                logger.info(f"[VOICE_PROXY] Forwarding event={event_type}")
                        except Exception:
                            pass
                    ...
                    logger.error(f"[VOICE_WS] Client -> Backend error: {e}")
            ...
            async def backend_to_client():
                try:
                    ...
                    logger.error(f"[VOICE_WS] Backend -> Client error: {e}")
        ...
        logger.error(f"[VOICE_WS] Connection to backend failed: {e}")
