...
class SensorManager {
  ...
  final StreamController<SensorMeasurement> _measurementController = StreamController<SensorMeasurement>.broadcast();
  Stream<SensorMeasurement> get measurementStream => _measurementController.stream;
  ...
  Future<void> startSensor(SensorType type) async {
    ...
    if (provider == null) {
      print('[EXPERIMENT] SENSOR_ERROR provider=${type.name} message=Provider not found');
      return;
    }

    if (!await provider.isAvailable()) {
      print('[EXPERIMENT] SENSOR_ERROR provider=${type.name} message=Sensor is unavailable');
      return;
    }

    await provider.start();
    _subscriptions[type] = provider.measurementStream.listen((measurement) {
