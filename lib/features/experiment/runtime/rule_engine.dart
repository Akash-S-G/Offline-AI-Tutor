...
class RuleEngine {
  ...
  RuleEngine(this._variableStore, this._eventBus)
    : _mathEvaluator = MathEvaluatorService();
  ...
  List<Map<String, dynamic>> get allRules => List.unmodifiable(_rules);

  void evaluateContinuousRules(double dt) {
    // Basic contextual variables
    final Map<String, dynamic> contextVars = Map.from(
    ...
    if (conditionMet && rule.containsKey('action')) {
      ...
      if (action is String) {
        if (action.contains('=')) {
          ...
          final targetVar = parts[0]
              .replaceAll('+', '')
              .replaceAll('-', '')
              .trim();
          final exprStr = parts[1].trim();
          try {
            final result = _mathEvaluator.evaluateExpression(
              exprStr,
              contextVars,
            );
