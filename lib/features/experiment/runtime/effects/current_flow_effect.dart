...
/// Reads: particle_speed, particle_density (from visual mapping).
///
/// Params:
...
class CurrentFlowEffect implements RuntimeEffect {
  ...
  void tick(EffectContext context, Map<String, dynamic> params) {
    final speed = context.prop('particle_speed', defaultValue: 0.0);
    final density = context.prop('particle_density', defaultValue: 20.0).toInt();

    if (speed <= 0.01 || density <= 0) return;
    ...
    for (int i = 0; i < density; i++) {
      final phase = (context.time * speed * 2.0 + i / density) % 1.0;
