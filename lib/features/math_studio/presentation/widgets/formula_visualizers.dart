...
class CircleVisualizationPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawCircle(center, maxRadius, Paint()..color = Colors.grey.withOpacity(0.2)..style = PaintingStyle.fill);

    // Dynamic radius drawing
    final drawRadius = math.min(math.max(radius, 5.0), maxRadius);
    final fillPaint = Paint()..color = const Color(0xFF8B5CF6).withOpacity(0.5)..style = PaintingStyle.fill;
    final strokePaint = Paint()..color = const Color(0xFF6D28D9)..style = PaintingStyle.stroke..strokeWidth = 3;
  ...
  @override
  bool shouldRepaint(covariant CircleVisualizationPainter oldDelegate) => oldDelegate.radius != radius;
}
...
class PythagoreanPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fillPaint = Paint()..color = const Color(0xFF8B5CF6).withOpacity(0.3)..style = PaintingStyle.fill;
    final strokePaint = Paint()..color = const Color(0xFF6D28D9)..style = PaintingStyle.stroke..strokeWidth = 3;
    ...
    // Right angle marker
    canvas.drawLine(p1 + const Offset(10, 0), p1 + const Offset(10, -10), strokePaint);
    canvas.drawLine(p1 + const Offset(10, -10), p1 + const Offset(0, -10), strokePaint);
  }

  @override
  bool shouldRepaint(covariant PythagoreanPainter oldDelegate) => oldDelegate.a != a || oldDelegate.b != b;
}
...
class InterestPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Principal Rect
    canvas.drawRect(
      Rect.fromLTRB(gap, size.height - (principal * scale), gap + pWidth, size.height),
      pPaint,
    );
    ...
    canvas.drawRect(
      Rect.fromLTRB(start2, size.height - (principal * scale), start2 + iWidth, size.height),
      pPaint,
    );
    canvas.drawRect(
      Rect.fromLTRB(start2, size.height - (total * scale), start2 + iWidth, size.height - (principal * scale)),
      iPaint,
    );
  }
  ...
  bool shouldRepaint(covariant InterestPainter oldDelegate) => 
    oldDelegate.principal != principal || oldDelegate.rate != rate || oldDelegate.time != time;
}
...
class SpeedPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(Offset(0, trackY), Offset(size.width, trackY), Paint()..color = Colors.grey..strokeWidth = 4);
    
    // Simulate position (normalize distance to width)
    final dx = math.min(distance, 100.0) / 100.0 * size.width;
    
    canvas.drawCircle(Offset(dx, trackY), 12, Paint()..color = const Color(0xFF8B5CF6));
  }

  @override
  bool shouldRepaint(covariant SpeedPainter oldDelegate) => oldDelegate.distance != distance || oldDelegate.time != time;
}
...
class PercentagePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawRRect(RRect.fromRectAndRadius(barRect, const Radius.circular(10)), Paint()..color = Colors.grey.shade300);

    final ratio = math.min(math.max(part / whole, 0.0), 1.0);
    final fillRect = Rect.fromLTWH(0, size.height / 2 - 10, size.width * ratio, 20);
    canvas.drawRRect(RRect.fromRectAndRadius(fillRect, const Radius.circular(10)), Paint()..color = const Color(0xFF8B5CF6));
  }

  @override
  bool shouldRepaint(covariant PercentagePainter oldDelegate) => oldDelegate.part != part || oldDelegate.whole != whole;
}
